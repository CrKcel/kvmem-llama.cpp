...
bool kvmem_spec_start(kvmem_spec_session & sess,
    ...
    if (opts.n_max < 1 || opts.n_max >= (int) llama_n_batch(ctx_tgt) ||
            opts.n_max >= (int) llama_n_ubatch(ctx_tgt)) {
        fprintf(stderr, "kvmem_spec_start: draft count must be positive and smaller than batch/ubatch size\n");
        return false;
    }
...
kvmem_spec_gen_stats kvmem_spec_generate(
    ...
    kvmem_diag("KVMEM_GDN_PERF mode=snapshots verify_calls=%llu committed_rows=%llu verify_ms=%.3f\n",
            (unsigned long long) verify_calls,
            (unsigned long long) committed_rows, verify_us / 1000.0);
