...
The plain/KVMem comparison uses the same patched binary with KVMem disabled/enabled. It is not a comparison against pristine Prism, nor a token-logit equivalence test. Only simple text and one synthetic retrieval example have been validated. No claims are made for 32K/256K quality, general tool use, vision, ordinary Qwen regressions, other GPUs, Linux, or long-duration stability. Installed rc3 runtime files and the existing app were not changed.
