...
**状态（2026-09-06）：** P0–P3 **`v0.3.0`**。P4–P5 **`v0.4.0`**。P7 **`v0.5.0`**。量化 KV spill / unrotated q8 raw-K **`v0.6.0`**。GPU stage-in（dequant + orig-pos RoPE + Hadamard + q8）打成 **`v0.7.0`**（tag `v0.7.0`）。产品默认 `--kvmem` = retrieval + query-last 64，GPU KV **q8_0**；MTP 可选 `--spec-type draft-mtp`（默认 **none**）。ISTA IQ3_S 不开 MTP。阶段测试：[milestones/v0.7.0.md](milestones/v0.7.0.md)。P6 未开始。
